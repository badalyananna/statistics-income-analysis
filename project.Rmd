...
```{r}
reg.out <- lm(log(realincwage) ~ . -numprec, data = data.reg[train,], weights=numprec)
summary(reg.out)
...
```{r}
library(leaps)
regfit.fwd <- regsubsets(log(realincwage) ~ . -industry-occupation-classwkr, data=data.reg, weights = data.reg$numprec, method="forward", nvmax=100)
...
n.bwd <- which.min(bwd.summary$bic)
points(n.bwd,bwd.summary$bic[n.bwd],col="red",cex=2,pch=20)
```
```{r}
...
X <- model.matrix(realincwage ~ . -numprec, data = data.reg)
X <- X[,-1]
y <- data.reg$realincwage
#grid <- 10^seq(10, -2, length=100)

ridge.mod <- glmnet(X, y, alpha=0, weights=data.reg$numprec)
plot(ridge.mod, label=TRUE)

```
```{r}
cv.out <- cv.glmnet(X[train, ], y[train], alpha = 0, nfold=10, weights=data.reg$numprec[train], type.measure = "mse")
plot(cv.out)
```
...
```{r}
out <- glmnet(X, y, alpha = 0, weights = data.reg$numprec)
